  ...
  try {
    ...
    const emailResponse = await resend.emails.send({
      ...
      html: `
        ...
        <h3>Schedule</h3>
        <ul>
          <li><strong>Pickup:</strong> ${escapeHtml(new Date(data.pickupDateTime).toLocaleString('en-US', { dateStyle: 'medium', timeStyle: 'short' }))}</li>
          <li><strong>Return:</strong> ${escapeHtml(new Date(data.returnDateTime).toLocaleString('en-US', { dateStyle: 'medium', timeStyle: 'short' }))}</li>
