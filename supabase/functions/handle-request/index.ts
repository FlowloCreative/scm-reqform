  ...
  try {
    ...
    const emailResponse = await resend.emails.send({
      ...
      html: `
        ...
        <h3>Schedule</h3>
        <ul>
          <li><strong>Pickup:</strong> ${escapeHtml(new Date(data.pickupDateTime).toLocaleDateString('en-US', { timeZone: 'UTC', dateStyle: 'medium' }))}</li>
          <li><strong>Return:</strong> ${escapeHtml(new Date(data.returnDateTime).toLocaleDateString('en-US', { timeZone: 'UTC', dateStyle: 'medium' }))}</li>
